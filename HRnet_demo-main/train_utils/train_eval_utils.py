...
def train_one_epoch(model, optimizer, data_loader, device, epoch,
    ...
    # 使用tqdm创建进度条
    pbar = tqdm(enumerate(data_loader), total=len(data_loader), desc=header)
    for i, [images, targets] in pbar:
        ...
        # 更新进度条信息
        if i % print_freq == 0:
            pbar.set_postfix({'损失': f'{loss_value:.4f}', '学习率': f'{now_lr:.6f}'})
...
def evaluate(model, data_loader, device, flip=False, flip_pairs=None):
    if flip:
        assert flip_pairs is not None, "启用翻转时必须提供 flip_pairs。"
    ...
    for image, targets in pbar:
        ...
        # 更新进度条信息
        pbar.set_postfix({'模型时间': f'{model_time:.4f}'})
