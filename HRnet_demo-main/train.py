...
def create_model(num_joints, load_pretrain_weights=True):
    ...
    if load_pretrain_weights:
        ...
        weights_dict = torch.load("E:\daChuangHRNet\pytorch_keypoint\pytorch_keypoint\HRNet\weights\hrnet_w32.pth", map_location='cpu')
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        with open(results_file, "a") as f:
            ...
            txt = f"轮次:{epoch} {' '.join(result_info)}"
            f.write(txt + "\n")
        ...
        torch.save(save_files, f"./save_weights/model-{epoch}.pth")

        print(f"第 {epoch} 轮训练完成。平均损失: {mean_loss:.4f}, 学习率: {lr:.6f}")
        print(f"验证集 mAP@0.5: {coco_info[1]:.4f}")
...
if __name__ == "__main__":
    ...
    # 训练数据集的根目录(coco2017)
    parser.add_argument('--data-path', default='E:\daChuangHRNet\pytorch_keypoint\pytorch_keypoint\HRNet\data\coco2017',
                        help='数据集路径')
    # COCO数据集人体关键点信息
